...
class RuntimeContext:
    ...
    def resolved_server_args_dict(self, base: dict | None = None) -> dict:
        ...
        ``get_internal_state`` (and through it the ``internal_states`` block of
        ``/server_info``) surfaces the config the process is *currently*
        running, not the startup record, so it reads this rather than
        serializing ``server_args`` directly — otherwise scheduler-side runtime
        changes (HiCache attach/detach, the generated forward-pass-metrics
        endpoint, tunables set via ``/set_internal_state``) never show up.
