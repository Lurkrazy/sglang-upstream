...
- **Moving code drops first-line guards**: early returns (`if self.is_draft_worker: return`)
  ...
  instance is private either way: the scheduler builds it with
  `draft_server_args_copy()` before the worker factory, and
  `build_draft_tp_worker()` deepcopies again for dflash / dspark.
